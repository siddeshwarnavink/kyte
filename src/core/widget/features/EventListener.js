    ...
    run() {
        this.forEachAttrs(currentAttribute => {
            if (currentAttribute.name.charAt(0) === '#') {
                ...
                if (!this.widgetInst._eventListeners.find(event => (event.type === eventName && event.element.innerHTML.trim() === this.childEl.innerHTML.trim()))) {
                    ...
                    if (eventName === 'bind') {
                        ...
                        if (htmlContains(this.widgetInst.template, this.childEl.outerHTML)) {
                            ...
                            const statePieceName = currentAttribute.value.substring(1, currentAttribute.value.length - 1).replace('this.', 'this.widgetInst.');
                    ...
                    // Default event listener
                        ...
                        const eventHandler = () => {
                            const codeToRun = `${currentAttribute.value.substring(1, currentAttribute.value.length - 1).replace('this.', 'this.widgetInst.')}(event)`;
                            eval(codeToRun);
                        };
